...
const cityData: Record<string, City> = {
  manila: {
    ...
    witnesses: [
      {
        name: 'Budget Beat Reporter',
        role: 'Press',
        spot: 'Quezon City newsroom',
        avatar: '🗞️',
      },
      {
        name: 'Airport Ramp Clerk',
        role: 'Aviation',
        spot: 'VIP apron',
        avatar: '🛫',
      },
      {
        name: 'Flood Project Engineer',
        role: 'Whistleblower',
        spot: 'Drainage site',
        avatar: '🧰',
      },
  ...
  london: {
    ...
    tagline:
      'Red buses, rain clouds, and bankers who blink twice before answering.',
    color: '#44b3ff',
    x: 36,
    y: 26,
    ...
    witnesses: [
      ...
      },
      {
        name: 'Archive Clerk',
        role: 'Records',
        spot: 'Companies registry desk',
        avatar: '📇',
      },
      {
        name: 'Cab Dispatcher',
        role: 'Transport',
        spot: 'Victoria station',
        avatar: '🚕',
      },
    ],
    flavor: [
  ...
  paris: {
    ...
    country: 'France',
    tagline:
      'Macarons, metro maps, and a briefcase changing hands near the Seine.',
    color: '#ff5fab',
    x: 41,
    y: 39,
    ...
    witnesses: [
      {
        name: 'Cafe Owner',
        role: 'Local Tipster',
        spot: 'Left Bank terrace',
        avatar: '☕',
      },
      {
        name: 'Gallery Guard',
        role: 'Security',
        spot: 'Private viewing room',
        avatar: '🖼️',
      },
      {
        name: 'Metro Busker',
        role: 'Street Witness',
        spot: 'Chatelet platform',
        avatar: '🎷',
      },
    ],
    flavor: [
  ...
  amsterdam: {
    ...
    witnesses: [
      {
        name: 'Canal Boat Captain',
        role: 'Guide',
        spot: 'Prinsengracht dock',
        avatar: '⛵',
      },
      {
        name: 'Bike Courier',
  ...
  geneva: {
    ...
    country: 'Switzerland',
    tagline:
      'Alpine chocolate, watch shops, and private ledgers with polite smiles.',
    color: '#7b61ff',
    x: 48,
    y: 49,
  ...
  madrid: {
    ...
    country: 'Spain',
    tagline:
      'Sunny plazas, late dinners, and a red scarf left at a hotel desk.',
    color: '#ffb000',
    x: 34,
    y: 63,
    ...
    witnesses: [
      ...
      },
      {
        name: 'Museum Docent',
        role: 'Culture',
        spot: 'Private tour wing',
        avatar: '🎨',
      },
      {
        name: 'Station Cleaner',
        role: 'Rail Staff',
        spot: 'Atocha concourse',
        avatar: '🧹',
      },
    ],
    flavor: [
  ...
  rome: {
    ...
    witnesses: [
      {
        name: 'Scooter Mechanic',
        role: 'Street Lead',
        spot: 'Trastevere garage',
        avatar: '🛵',
      },
      {
      ...
      {
        name: 'Airport Caterer',
        role: 'Aviation',
        spot: 'Ciampino service gate',
        avatar: '🥐',
      },
    ],
    flavor: [
    ...
    witnesses: [
      {
        name: 'Ferry Agent',
        role: 'Harbor Staff',
        spot: 'Piraeus ticket booth',
        avatar: '⛴️',
      },
      {
        name: 'Rooftop Host',
        role: 'Hotel Staff',
        spot: 'Monastiraki terrace',
        avatar: '🏨',
      },
      {
        name: 'Antique Seller',
...
const createCase = (): CaseFile => {
  ...
  return {
    ...
    dossier: {
      identity:
        'A well-known Filipino political figure using aides, initials, and hotel aliases softer than a committee subpoena.',
      vehicle:
        'A private aircraft trail with luxury service invoices, VIP apron access, and baggage tags that forgot to plead the Fifth.',
      hideout: `A reserved suite and security detail near ${cityData[finalCity].name}.`,
      motive:
        'Avoiding public inquiries tied to alleged flood-control budget irregularities and mysteriously dry paper trails.',
    },
  }
}
...
const getMentionedCityIds = (text: string): string[] =>
  Object.values(cityData)
    .filter((city) =>
      new RegExp(`\\b${escapeRegExp(city.name)}\\b`, 'i').test(text),
    )
    .map((city) => city.id)
...
const Game = () => {
  ...
  useEffect(() => {
    ...
    ;(Object.keys(audioAssets) as Array<keyof typeof audioAssets>).forEach(
      (key) => {
        if (key === 'music') return
  ...
  const clueLeadCityIds = Array.from(
    new Set(
      gameState.log
  ...
  const travelOptions = Array.from(
    new Set([...currentCity.connections, ...clueLeadCityIds]),
  )
  const hasRouteLead =
    gameState.log.some(
      (entry) => entry.type === 'route' && entry.city === currentCity.name,
  ...
  const interviewWitness = (witness: Witness) => {
    ...
    const unknownEvidence = evidenceKeys.filter(
      (key) => !gameState.evidence[key],
    )
    const shouldGiveRoute = Math.random() < 0.58 || unknownEvidence.length === 0
    const type: EvidenceKey | 'route' | 'decoy' = shouldGiveRoute
      ? 'route'
      : pick(unknownEvidence)
    const clue =
      type === 'route'
        ? getRouteClue(caseFile, gameState.currentCity)
        : getEvidenceClue(caseFile, type)
    const nextEvidence =
      type !== 'route' && type !== 'decoy'
      ...
      setGameState(
        resetTypewriter({
          ...
          conversation: {
            ...
            text: 'The chase clock hit zero. The suspect slipped through a private door while the paperwork was still warming up. Your clues will stay on file, and the Europe desk will be ready for another run.',
            avatar: '🧊',
          },
        }),
      )
  ...
  const useGumdropAssist = () => {
    ...
    const nextStop =
      trailIndex >= 0 && trailIndex < caseFile.trail.length - 1
        ? cityData[caseFile.trail[trailIndex + 1]].name
        : currentCity.name
    const clue =
      trailIndex === caseFile.trail.length - 1
      ...
      setGameState(
        resetTypewriter({
          ...
          conversation: {
            ...
            text: 'Clean warrant, correct city, excellent detective work. The fugitive is boxed in by passport stamps, witness notes, and a paper trail bright enough to see from the departures board. The case file now moves from cartoon chase to the serious business of lawful accountability.',
            avatar: '🏆',
          },
        }),
      )
      ...
      <main className="game-shell loading-mode">
        ...
        <section className="loading-screen" aria-label="Loading game">
          ...
          <p className="loading-copy">
            Polishing glossy buttons · stamping passports · warming up the audit
            trail
          </p>
        </section>
      </main>
  ...
  return (
    <main className="game-shell">
      ...
      <section className="topbar">
        ...
        <div className="topbar-hud">
          <div className="score-strip" aria-label="Case meters">
            <div className="hud-token days">
              <b>⏱</b>
              <span>{gameState.daysLeft}</span>
              <small>Days</small>
            </div>
            <div className="hud-token clues">
              <b>🔎</b>
              <span>{evidenceCount}/4</span>
              <small>Clues</small>
            </div>
            <div className="hud-token assists">
              <b>📡</b>
              <span>{gameState.gumdrops}</span>
              <small>Boosts</small>
            </div>
            <div className="hud-token heat">
              <b>🔥</b>
              <span>{gameState.heat}</span>
              <small>Heat</small>
            </div>
          </div>
          <button
            ...
            onClick={toggleAudio}
            aria-label={audioEnabled ? 'Mute game audio' : 'Unmute game audio'}
      ...
      <section className={`game-board mobile-panel-${mobilePanel}`}>
              ...
              {gameState.showTravel ? (
                <div className="route-list">
                  {travelOptions.map((cityId) => {
                    ...
                    const hot =
                      trailIndex >= 0 &&
                      caseFile.trail[trailIndex + 1] === cityId
                    const clueLead =
                      clueLeadCityIds.includes(cityId) &&
                      !currentCity.connections.includes(cityId)

                    return (
                      <button
                        key={cityId}
                        onClick={() => askTravelTo(cityId)}
                        className={`${hot ? 'hot-route ' : ''}${clueLead ? 'lead-route ' : ''}next-action-list`}
                      >
        ...
        <div
          className={`map-card ${gameState.showTravel ? 'route-mode' : 'clue-mode'}`}
        >
          ...
          <div className="mini-map">
            ...
            {Object.values(cityData).map((city) => {
              ...
              const clueLead =
                clueLeadCityIds.includes(city.id) &&
                !currentCity.connections.includes(city.id)
          ...
          <div className="passport-row">
            {gameState.visited.slice(-7).map((cityId, index) => (
              <span key={`${cityId}-${index}`} className="stamp">
                {cityData[cityId].name}
              </span>
            ))}
          </div>
        </div>
        ...
        <aside className="dossier-card hud-panel right-hud">
          ...
          <div className="warrant-status">
            <strong>
              {gameState.warrantReady ? 'Warrant Ready' : 'Build Warrant'}
            </strong>
            <span>
              {hasEnoughEvidence
                ? '3+ evidence cards collected'
                : `${3 - evidenceCount} more evidence card${3 - evidenceCount === 1 ? '' : 's'} needed`}
            </span>
          </div>
          <div className="evidence-grid">
            {(Object.keys(evidenceLabels) as EvidenceKey[]).map((key) => (
              <article
                key={key}
                className={gameState.evidence[key] ? 'filled' : ''}
              >
          ...
          <div className="clue-log">
            ...
            {gameState.log.length === 0 ? (
              <p className="empty-log">
                No clues yet. Shake down the witness buttons.
              </p>
            ) : (
        ...
        {gameState.phase === 'active' && (
          <nav className="bottom-dock" aria-label="Primary game actions">
            <button
              className={`candy-button travel ${hasRouteLead && !gameState.showTravel ? 'next-action' : ''}`}
              onClick={toggleTravelMode}
            >
              <span>✈</span> {gameState.showTravel ? 'Routes' : 'Travel'}
            </button>
            <button
              className={`candy-button boost ${gameState.gumdrops > 0 && gameState.log.length === 0 ? 'next-action' : ''}`}
              onClick={useGumdropAssist}
              disabled={gameState.gumdrops <= 0}
            >
              <span>📡</span> Scan
            </button>
            <button
              className={`candy-button danger ${gameState.warrantReady ? 'next-action' : ''}`}
        ...
        <nav className="mobile-tabbar" aria-label="Mobile panel navigation">
          <button
            className={mobilePanel === 'interview' ? 'active' : ''}
            onClick={() => switchMobilePanel('interview')}
          >
            <span>🗣</span>
            Interview
          </button>
          <button
            ...
            onClick={() => switchMobilePanel('map')}
          >
            <span>🗺</span>
            Map
          </button>
          <button
            className={mobilePanel === 'clues' ? 'active' : ''}
            onClick={() => switchMobilePanel('clues')}
          >
            <span>🔎</span>
            Clues
          </button>
          ...
          <section className="how-to-modal">
            ...
            <div className="how-to-panel" aria-label="How to play">
              ...
              <div className="tutorial-actions">
                <button
                  type="button"
                  className="candy-button travel next-action"
                  aria-label="Travel button preview"
                >
                  <span>✈</span> Travel
                </button>
                <button
                  type="button"
                  className="candy-button boost"
                  aria-label="Scan button preview"
                >
                  <span>📡</span> Scan
                </button>
                <button
                  type="button"
                  className="candy-button danger"
                  aria-label="Warrant button preview"
                >
                  <span>⚖</span> Warrant
                </button>
              </div>
              <p>
                Interview witnesses, click luggage on the map to travel, and
                watch for named cities in clues: they unlock special clue-lead
                routes. Use Scan if the audit trail goes cold, then issue the
                Warrant after collecting 3 evidence cards.
              </p>
              <button
                className="candy-button primary next-action"
      ...
      {pendingTravel && (
        <div
          className="modal-backdrop travel-confirm-backdrop"
          role="dialog"
          aria-modal="true"
        ...
        >
          <section className="travel-confirm-modal">
            ...
            <div>
              ...
              <h2 id="travel-confirm-title">
                Fly to {cityData[pendingTravel].name}?
              </h2>
              <p>
                This spends 1 day and stamps your passport. Double-check the
                clue: a wrong hop adds heat and gives the alleged flood-fund
                travel circus more time to change gates.
              </p>
